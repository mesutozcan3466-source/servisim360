...
class _SoforPanelScreenState extends State<SoforPanelScreen> {
  ...
  // ── Sürücü dokümanı bul ──────────────────────────────────────
  Future<DocumentSnapshot?> _surucuDokumaniBul(
  ...
  Future<void> _yukle() async {
    ...
    try {
      ...
      if (driverDoc != null) {
        ...
        _aktifProjeId  = SessionService.instance.aktifProjeld
            ?? _driverDoc['aktifProjeId']  as String?;
        _aktifProjeAdi = SessionService.instance.aktifProjeAdi
            ?? _driverDoc['aktifProjeAdi'] as String?;
    ...
    if (_surucuId.isNotEmpty) {
      ...
      // Arka plan servisi — yeni API ile
      await ArkaplanKonumServisi.baslatServisi();
      if (_firmaId != null) {
        await ArkaplanKonumServisi.baslat(
          surucuDocId: _surucuId,
          firmaId: _firmaId!,
        );
      }
    }
    ...
    try {
      ...
      for (final doc in snap.docs) {
        ...
        if (atanmis || (surucular == null && projeIds == null)) {
          liste.add({
            'id': doc.id,
            'ad': data['ad'] ?? data['projeAdi'] ?? doc.id,
            ...data,
          });
        }
      }
  ...
  Future<void> _servisBaslatDurdur() async {
    if (_surucuId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Sofor kaydi bulunamadi'),
    ...
    if (yeniDurum) {
      ...
      _gpsDurdur();
      await ArkaplanKonumServisi.durdur(surucuDocId: _surucuId);
      await FirebaseFirestore.instance
          .collection('drivers').doc(_surucuId).update({
        'servisAktif': false,
        'servisBitis': FieldValue.serverTimestamp(),
  ...
  Future<void> _navigasyonAc() async {
    if (_ogrenciler.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Atanmis ogrenci bulunamadi'),
          backgroundColor: Colors.orange));
      return;
    }
    final ilk = _ogrenciler.firstWhere(
            (o) => !(o['bindi'] ?? false),
        orElse: () => _ogrenciler.first);
  ...
  void _rotaYonetimine() {
    ...
    Navigator.push(context, MaterialPageRoute(
        builder: (_) => SoforRotaScreen(
            surucuId: _surucuId, surucuAd: ad))).then((_) => _yukle());
  }
  ...
  void _projeSecimAc() {
    if (_projeler.length <= 1) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Size atanmis baska proje yok'),
          backgroundColor: Colors.orange));
      return;
    }
    showModalBottomSheet(
              ...
              child: Container(
                ...
                child: Row(children: [
                  Icon(Icons.folder_outlined,
                      color: secili ? Colors.white : _navy, size: 20),
          ...
          body: Center(
              child: Padding(
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text(
                        'Hesabiniz henuz bir sofor profiline baglanmamis.\n'
              ...
              Row(children: [
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          Row(children: [
                            ...
                            Text(
                                _servisAktif ? 'Servis Aktif' : 'Hazir',
                                style: const TextStyle(
                ...
                IconButton(
                    ...
                    onPressed: () => Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (_) => const SifreDegistirScreen(
                                rol: 'sofor')))),
                IconButton(
                    icon: const Icon(Icons.logout_outlined,
                        color: Colors.white70),
                ...
                child: Column(children: [
                  ...
                  if (_aiAcik) ...[
                    SoforAiAsistanWidget(
                      ...
                      servisDurumu:
                      _servisAktif ? 'basladi' : 'bekleniyor',
                      sabahBaslangic:
                      _driverDoc['servisSaati']?['sabahBaslangic'] ??
                          '06:30',
                      sabahBitis:
                      _driverDoc['servisSaati']?['sabahBitis'] ?? '09:30',
                      aksamBaslangic:
                      _driverDoc['servisSaati']?['aksamBaslangic'] ??
                          '15:00',
                      aksamBitis:
                      _driverDoc['servisSaati']?['aksamBitis'] ?? '18:30',
                    ),
                    const SizedBox(height: 12),
                  ],

                  // Servisi Baslat/Durdur
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                        style: ElevatedButton.styleFrom(
                            backgroundColor:
                            _servisAktif ? Colors.red : Colors.green,
                            foregroundColor: Colors.white,
                            elevation: 6,
                  ...
                  Row(children: [
                    ...
                    Expanded(child: _BuyukButon(
                        ikon: Icons.route,
                        etiket: 'Rota\n${_ogrenciler.length} kisi',
                        renk: _navy,
                        onTap: _rotaYonetimine)),
                  ]),
                  const SizedBox(height: 12),
                        ...
                        child: Row(
                            ...
                            children: [
                              ...
                              _MiniIstat(
                                  ...
                                  Colors.orange),
                              _MiniIstat('${_ogrenciler.length}', 'Toplam',
    ...
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      if (ogrenciler.isEmpty)
        const Padding(
            ...
            child: Text('Atanmis ogrenci yok',
                style: TextStyle(color: Colors.grey)))
      else
        ...ogrenciler.map((ogr) {
          final tel =
              ogr['veliTel'] ?? ogr['veliTelefon'] ?? '';
          return ListTile(
              leading: CircleAvatar(
                  backgroundColor: _navy.withValues(alpha: 0.1),
                  child: Text(
                      (ogr['ad'] ?? '?')[0].toUpperCase(),
                      style: const TextStyle(
                          color: _navy,
                          fontWeight: FontWeight.bold))),
              title: Text(ogr['ad'] ?? '',
                  style: const TextStyle(
                      fontWeight: FontWeight.bold)),
              subtitle: Text(
                  tel.isNotEmpty ? tel : 'Telefon yok',
